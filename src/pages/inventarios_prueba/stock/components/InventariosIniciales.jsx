...
const InventarioInicial = ({ open, onClose, idInsumo, insumoData = null }) => {
    ...
    const fetchDataStockBodegaInsumo = async () => {
        ...
        setLoading(true);
        try {
            const response = await axiosClient.get(`/inventario/stock/bodega-insumo/${idInsumo}`);
            const rows = Array.isArray(response?.data) ? response.data : response?.data?.data || [];
            setDataStockBodegaInsumo(rows);
        } catch {
    ...
    const buscarBodega = async (id_sede, id_facultad) => {
        ...
        try {
            const resp = await axiosClient.get(
                `/inventario/bodegas/${Number(id_sede)}/${Number(id_facultad)}`
            );
            const payload = resp?.data;
            const bodegas = Array.isArray(payload) ? payload : payload?.data ?? [];
    ...
    const handleGuardarInventarioInicial = async () => {
        ...
        try {
            const { data } = await axiosClient.post(
                "/inventario/stock/inventario-inicial",
                payload
            );
    ...
    const handleFetchDataStockBodegaInsumoId = async (id) => {
        ...
        try {
            const response = await axiosClient.get(
                `/inventario/stock/bodega-insumo-detalle/${id}`
            );
    ...
    const handleVerLotesBodega = async (sbId) => {
        ...
        try {
            const response = await axiosClient.get(
                `/inventario/stock/bodega-insumo-detalle/${sbId}`
            );
            const payload = response?.data;
            const row = Array.isArray(payload) ? payload[0] : payload;
