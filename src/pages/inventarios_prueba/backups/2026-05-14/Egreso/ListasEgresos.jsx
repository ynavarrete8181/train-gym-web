...
function Egresos() {
  ...
  const exportExcel = async () => {
    ...
    const mapped = rows.map((r) => ({
      ...
      Carrera: r.carrera_nombre ?? r.bodega_id_carrera ?? "",
  ...
  const exportPdf = async () => {
    ...
    const head = [
      ["ID", "Nro Egreso", "Fecha", "Cédula", "Paciente", "Médico", "Entrega", "Sede", "Facultad", "Bodega", "Estado"],
    ];
    ...
    const body = rows.map((r) => [
      ...
      safeStr(r.cedula_paciente ?? ""),
      safeStr(r.nombre_paciente ?? ""),
      safeStr(r.nombre_funcionario ?? ""),
      safeStr(r.nombre_usuario ?? ""),
      safeStr(r.sede_nombre ?? ""),
      safeStr(r.facultad_nombre ?? ""),
          ...
          <table style="width:100%; border-collapse:collapse; font-size:12px;">
            <tbody>
              ...
              <tr>
                ...
                <td style="padding:8px; border:1px solid #e6ebf2;">
                  ${esc(row.nombre_paciente)} &nbsp; | &nbsp; Cédula: ${esc(row.cedula_paciente)}
                </td>
              </tr>
              <tr>
              ...
              <tr>
                <td style="padding:8px; border:1px solid #e6ebf2; font-weight:700; background:#f6f8fc;">Sede</td>
                <td style="padding:8px; border:1px solid #e6ebf2;">${esc(row.sede_nombre || row.bodega_id_sede || "—")}</td>
              </tr>
              <tr>
                <td style="padding:8px; border:1px solid #e6ebf2; font-weight:700; background:#f6f8fc;">Facultad</td>
                <td style="padding:8px; border:1px solid #e6ebf2;">${esc(row.facultad_nombre || row.bodega_id_facultad || "—")}</td>
              </tr>
              <tr>
                <td style="padding:8px; border:1px solid #e6ebf2; font-weight:700; background:#f6f8fc;">Carrera</td>
                <td style="padding:8px; border:1px solid #e6ebf2;">${esc(row.carrera_nombre || row.bodega_id_carrera || "—")}</td>
              </tr>
              <tr>
                <td style="padding:8px; border:1px solid #e6ebf2; font-weight:700; background:#f6f8fc;">Bodega</td>
                <td style="padding:8px; border:1px solid #e6ebf2;">${esc(row.bodega_nombre || row.ee_id_bodega || "—")}</td>
              </tr>
              <tr>
  ...
  return (
    <Box sx={{ ...baseFontSx, minHeight: "100%", bgcolor: ui.bg, p: 1.5 }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.25, flexWrap: "wrap" }}>
            ...
            <Button
              ...
              sx={{
                ...
                color: "#d32f2f",
                fontWeight: 700,
                "&:hover": { bgcolor: "rgba(211,47,47,0.08)", borderColor: "#b71c1c", color: "#b71c1c" },
              }}
            >
              PDF
            </Button>
            ...
            <Button
              ...
              sx={{
                ...
                color: "#2e7d32",
                fontWeight: 700,
                "&:hover": { bgcolor: "rgba(46,125,50,0.08)", borderColor: "#1b5e20", color: "#1b5e20" },
              }}
            >
              Excel
            </Button>
          ...
          <Grid container spacing={1} alignItems="center">
            ...
            <Grid item xs={6} md={3}>
              <FormControl size="small" fullWidth sx={fieldSx}>
                ...
                <Select labelId="estado-label" label="Estado" value={fEstado} onChange={(e) => setFEstado(e.target.value)}>
            ...
            <Grid item xs={12} md={4} sx={{ display: "flex", justifyContent: "flex-end" }}>
              <FormControlLabel
                ...
                label={<Typography sx={{ ...baseFontSx, fontSize: 12, color: ui.muted }}>Vista compacta</Typography>}
              />
          ...
          <Box sx={{ mt: 1.5 }}>
            ...
            >
              <Table stickyHeader size="small" sx={tableSx}>
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ minWidth: 170 }}>Nro Egreso</TableCell>
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell align="center">
                            <Tooltip
                              arrow
                              title={canVerAtencion ? "Ver atención" : "Disponible solo cuando el estado es 'Atendido' o 'No Asistió'"}
                            >
                              <span>
                                <IconButton
                                  ...
                                  size="small"
                                  sx={{ width: 40, height: 40, p: 0, borderRadius: 1, color: canVerAtencion ? "#1d4ed8" : "#94a3b8" }}
                                >
                            ...
                            <Tooltip arrow title={canAprobar ? "Aprobar / Atender egreso" : "No disponible"}>
                              <span>
                                <IconButton
                                  ...
                                  size="small"
                                  sx={{ width: 40, height: 40, p: 0, borderRadius: 1, color: canAprobar ? "#166534" : "#94a3b8" }}
                                >
      ...
      {openModalAtencionEgreso && egresoSeleccionadoId && (
        <CompAtencioEgreso open={openModalAtencionEgreso} onClose={handleCloseModalAtencionEgreso} idEgreso={egresoSeleccionadoId} />
      )}
    </Box>
  );
}
