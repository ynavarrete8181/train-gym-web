...
const estadosVenta = ['PENDIENTE', 'PARCIAL', 'PAGADA', 'ANULADA'];
...
const configs = {
  cajas: {
    titulo: 'Cajas',
    singular: 'Caja',
    descripcion: 'Administra puntos de cobro y cajas operativas.',
    icono: <PointOfSaleOutlinedIcon />,
    obtener: 'obtenerCajas',
    crear: 'crearCaja',
    actualizar: 'actualizarCaja',
    inicial: { id: null, sede_id: '', codigo: '', nombre: '', descripcion: '', saldo_inicial: 0, activa: true },
  },
  ventas: {
    titulo: 'Ventas',
    singular: 'Venta',
    descripcion: 'Registra ventas de productos, servicios y membresías.',
  ...
  pagos: {
    titulo: 'Pagos',
    singular: 'Pago',
    descripcion: 'Registra abonos y pagos confirmados de ventas.',
    icono: <PaymentsOutlinedIcon />,
    obtener: 'obtenerPagos',
    crear: 'crearPago',
    actualizar: null,
  ...
  comprobantes: {
    ...
    descripcion: 'Consulta recibos generados por ventas y pagos.',
    icono: <ReceiptLongOutlinedIcon />,
    obtener: 'obtenerComprobantes',
    crear: null,
    actualizar: null,
    inicial: {},
  },
    ...
    <Box className="page-wrapper">
      ...
      <Paper className="page-content-container" elevation={0}>
        <GestionToolbar
          ...
          acciones={config.crear ? <Button startIcon={<AddOutlinedIcon />} onClick={handleNuevo} sx={dbanuStyles.addButtonRevive}>Añadir</Button> : null}
        />
...
function Formulario({ tipo, formData, catalogos, onChange }) {
  ...
  if (tipo === 'ventas') {
    return (
      <Box sx={grid}>
        ...
        <TextField select label="Estado" name="estado" value={formData.estado || 'PENDIENTE'} onChange={onChange} required size="small">{estadosVenta.map((item) => <MenuItem key={item} value={item}>{item}</MenuItem>)}</TextField>
  ...
  return (
    <Box sx={grid}>
      ...
      <TextField select label="Estado" name="estado" value={formData.estado || 'CONFIRMADO'} onChange={onChange} required size="small"><MenuItem value="CONFIRMADO">CONFIRMADO</MenuItem><MenuItem value="ANULADO">ANULADO</MenuItem></TextField>
...
function columnasPorTipo(tipo, meta, filtros, onFiltro) {
  ...
  if (tipo === 'ventas') return [
    ...
    { key: 'estado', header: filtro('estado', 'Estado', opciones(estadosVenta)), render: (item) => <StatusChip estado={estadoTexto(item.estado)} /> },
  ];
  ...
  if (tipo === 'pagos') return [
    ...
    { key: 'estado', header: filtro('estado', 'Estado', [{ value: 'CONFIRMADO', label: 'Confirmado' }, { value: 'ANULADO', label: 'Anulado' }]), render: (item) => <StatusChip estado={estadoTexto(item.estado)} /> },
  ];
