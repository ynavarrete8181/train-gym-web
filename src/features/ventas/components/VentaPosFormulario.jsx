        ...
        return actual.map((fila) =>
          fila.clave === clave
            ? {
                ...
                total_linea: (fila.cantidad + 1) * fila.precio_unitario,
              }
            : fila,
        );
      }
    ...
    <Box className="page-wrapper">
      ...
      <Box
        sx={{
          ...
          gridTemplateColumns: {
            xs: '1fr',
            lg: 'minmax(280px, 0.85fr) minmax(500px, 1.8fr) minmax(260px, 0.75fr)',
          },
          gap: 2,
          alignItems: 'start',
        }}
      >
            ...
            {cliente ? (
              ...
              >
                ...
                <Typography variant="caption" color="text.secondary">
                  {cliente.codigo} · {cliente.telefono || 'Sin teléfono'}
                </Typography>
            ...
            {tipo !== 'OTRO' ? (
                ...
                <Stack spacing={1} sx={{ mt: 1.5, maxHeight: 430, overflowY: 'auto', pr: 0.5 }}>
                    ...
                    >
                      <Box sx={{ minWidth: 0 }}>
                        ...
                        <Typography
                          variant="body2"
                          fontWeight={700}
                          color={item.precio == null ? 'warning.main' : 'text.primary'}
                        >
                  ...
                  {disponibles.length === 0 ? (
                    <Typography variant="body2" color="text.secondary">
                      No hay ítems disponibles para este filtro.
                    </Typography>
                  ) : null}
                </Stack>
              </>
              ...
              <Stack spacing={1.25}>
                <TextField
                  ...
                  onChange={(e) => setOtro((a) => ({ ...a, descripcion: e.target.value }))}
                />
                <TextField
                  ...
                  size="small"
                  value={otro.precio}
                  onChange={(e) => setOtro((a) => ({ ...a, precio: e.target.value }))}
                />
                <TextField
                  label="Cantidad"
                  type="number"
                  size="small"
                  value={otro.cantidad}
                  onChange={(e) => setOtro((a) => ({ ...a, cantidad: e.target.value }))}
                />
                <Button startIcon={<AddShoppingCartOutlinedIcon />} variant="outlined" onClick={agregarOtro}>
                  Agregar al detalle
                </Button>
              </Stack>
            )}
          ...
          <Seccion titulo="2. Tipo de ítem">
            ...
            >
              {tipos.map((item) => (
                <ToggleButton key={item.value} value={item.value}>
                  {item.icono}
                  <Typography variant="caption" sx={{ ml: 0.7, fontWeight: 700 }}>
                    {item.label}
                  </Typography>
                </ToggleButton>
              ))}
            ...
            {tipo === 'MEMBRESIA' || tipo === 'PASE_DIARIO' ? (
              <Alert severity="info" sx={{ mt: 1.25, py: 0 }}>
                La asignación contractual se realiza desde Membresías; aquí se muestran sus planes y precios de referencia.
              </Alert>
            ) : null}
          </Seccion>
                  ...
                  <TableRow key={item.clave}>
                    ...
                    <TableCell>
                      <TextField
                        ...
                        size="small"
                        value={item.cantidad}
                        onChange={(e) => cambiarCantidad(item.clave, e.target.value)}
                        inputProps={{ min: 1, style: { padding: '6px 8px' } }}
                      />
                    </TableCell>
                    ...
                    <TableCell>
                      <IconButton
                        size="small"
                        color="error"
                        onClick={() => setCarrito((a) => a.filter((fila) => fila.clave !== item.clave))}
                      >
        ...
        <Stack spacing={2}>
          <Seccion titulo="Resumen">
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: 1, alignItems: 'center', mt: 1 }}>
            ...
            >
              ...
              <Typography variant="h5" fontWeight={800} color="primary.main">{dinero(total)}</Typography>
            </Box>
          </Seccion>
          ...
          <Seccion titulo="Cobro">
            ...
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1.25 }}>
              El cobro queda asociado automáticamente a {turno?.caja_nombre || 'la caja'} y al turno actual.
            </Typography>
          </Seccion>
        </Stack>
      </Box>
      ...
      <NotificacionSnackbar
        ...
        onClose={() => setNotificacion((a) => ({ ...a, mensaje: '' }))}
      />
    </Box>
  );
}
...
function Seccion({ titulo, children }) {
  return (
    <Paper elevation={0} sx={{ p: 1.75, border: '1px solid #e2e8f0', borderRadius: 2, bgcolor: '#fff' }}>
      <Typography variant="subtitle2" fontWeight={800} sx={{ mb: 1.25 }}>{titulo}</Typography>
      {children}
    </Paper>
  );
}
...
function DatoOperacion({ label, valor, destacado = false }) {
  return (
    <Box sx={{ px: 1.5, py: 0.5, minWidth: 0, borderRight: { md: '1px solid #e2e8f0' } }}>
      <Typography variant="caption" color="text.secondary" display="block">{label}</Typography>
      {destacado ? (
        <Chip size="small" label={valor} color="warning" variant="outlined" sx={{ mt: 0.5, fontWeight: 700 }} />
      ) : (
...
function ResumenFila({ label, valor }) {
  return (
    <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.55 }}>
