...
import { Alert, Autocomplete, Box, Button, Chip, Divider, IconButton, InputAdornment, MenuItem, Paper, Stack, Table, TableBody, TableCell, TableHead, TableRow, TextField, ToggleButton, ToggleButtonGroup, Tooltip, Typography } from '@mui/material';
...
export function VentaPosFormulario({ onVolver, onGuardado }) {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      try {
        ...
        avisar(error.response?.data?.mensaje || 'No se pudo cargar el contexto de venta.', 'error');
      } finally {
        setCargando(false);
      }
    };
  ...
  const subtotal = useMemo(() => carrito.reduce((total, item) => total + Number(item.total_linea || 0), 0), [carrito]);
  ...
  const disponibles = useMemo(() => {
    ...
    return items.filter((item) => `${item.nombre || ''} ${item.codigo || ''} ${item.categoria || ''}`.toLowerCase().includes(texto));
  ...
  const agregar = (item) => {
    ...
    setCarrito((actual) => {
      ...
      if (existe) {
        return actual.map((fila) => fila.clave === clave
  ...
  const cambiarCantidad = (clave, cantidad) => {
    ...
    setCarrito((actual) => actual.map((item) => item.clave === clave
  ...
  return (
    <Box className="page-wrapper">
      <PageHeader
        titulo="Nueva venta"
        descripcion="Punto de venta para servicios, productos y cobros de la sede del turno activo."
        icono={<PointOfSaleOutlinedIcon />}
        acciones={<BotonVolver onClick={onVolver} />}
      />

      {!cargando && !turno?.id ? (
        <Alert severity="warning" sx={{ mt: 2 }}>No tienes un turno de caja abierto. Abre un turno antes de registrar ventas.</Alert>
      ) : null}

      <Paper elevation={0} sx={{ mt: 2, p: 2, border: '1px solid #e2e8f0', borderRadius: 2 }}>
      ...
      <Box sx={{ mt: 2, display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(280px, 0.85fr) minmax(500px, 1.8fr) minmax(260px, 0.75fr)' }, gap: 2, alignItems: 'start' }}>
        <Stack spacing={2}>
          <Seccion titulo="1. Cliente">
            <Autocomplete
              ...
              renderInput={(params) => <TextField {...params} size="small" placeholder="Buscar por nombre o código" InputProps={{ ...params.InputProps, startAdornment: <><InputAdornment position="start"><SearchOutlinedIcon fontSize="small" /></InputAdornment>{params.InputProps.startAdornment}</> }} />}
            />
            {cliente ? (
              <Box sx={{ mt: 1.5, p: 1.5, border: '1px solid #e2e8f0', borderRadius: 1.5, bgcolor: '#f8fafc' }}>
            ...
            ) : <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>Puedes continuar como consumidor final.</Typography>}
          </Seccion>

          <Seccion titulo={`Catálogo · ${turno?.sede_nombre || 'sede actual'}`}>
            {tipo !== 'OTRO' ? (
              <>
                <TextField value={busqueda} onChange={(e) => setBusqueda(e.target.value)} size="small" fullWidth placeholder="Buscar ítem..." InputProps={{ startAdornment: <InputAdornment position="start"><SearchOutlinedIcon fontSize="small" /></InputAdornment> }} />
                <Stack spacing={1} sx={{ mt: 1.5, maxHeight: 430, overflowY: 'auto', pr: 0.5 }}>
                  {disponibles.map((item) => (
                    <Box key={`${tipo}-${item.id}`} sx={{ p: 1.25, border: '1px solid #e2e8f0', borderRadius: 1.5, display: 'flex', justifyContent: 'space-between', gap: 1, alignItems: 'center' }}>
                      <Box sx={{ minWidth: 0 }}>
                        <Typography variant="body2" fontWeight={700}>{item.nombre}</Typography>
                        <Typography variant="caption" color="text.secondary" display="block">{tipo === 'SERVICIO' ? `${item.duracion_minutos || 0} min${item.categoria ? ` · ${item.categoria}` : ''}` : item.codigo || item.descripcion || ''}</Typography>
                        <Typography variant="body2" fontWeight={700} color={item.precio == null ? 'warning.main' : 'text.primary'}>{item.precio == null ? 'Precio por configurar' : dinero(item.precio)}</Typography>
                      </Box>
                      <Tooltip title={(tipo === 'MEMBRESIA' || tipo === 'PASE_DIARIO') ? 'Se asigna desde Membresías' : 'Agregar'}>
                        <span><IconButton size="small" onClick={() => agregar(item)} disabled={item.precio == null}><AddShoppingCartOutlinedIcon fontSize="small" /></IconButton></span>
                      </Tooltip>
                    </Box>
                  ))}
        ...
        <Stack spacing={2}>
          <Seccion titulo="2. Tipo de ítem">
            <ToggleButtonGroup value={tipo} exclusive onChange={(_, value) => value && setTipo(value)} fullWidth size="small" sx={{ flexWrap: 'wrap', gap: 0.75, '& .MuiToggleButtonGroup-grouped': { border: '1px solid #d9e2ef !important', borderRadius: '8px !important', px: 1.25, flex: '1 1 120px' } }}>
              {tipos.map((item) => <ToggleButton key={item.value} value={item.value}>{item.icono}<Typography variant="caption" sx={{ ml: 0.7, fontWeight: 700 }}>{item.label}</Typography></ToggleButton>)}
            </ToggleButtonGroup>
            {(tipo === 'MEMBRESIA' || tipo === 'PASE_DIARIO') ? <Alert severity="info" sx={{ mt: 1.25, py: 0 }}>La asignación contractual se realiza desde Membresías; aquí se muestran sus planes y precios de referencia.</Alert> : null}
          </Seccion>

          <Seccion titulo="3. Detalle de la venta">
            <Table size="small">
              <TableHead><TableRow><TableCell>Descripción</TableCell><TableCell width={90}>Cant.</TableCell><TableCell align="right">P. Unit.</TableCell><TableCell align="right">Total</TableCell><TableCell width={45} /></TableRow></TableHead>
              <TableBody>
                {carrito.map((item) => <TableRow key={item.clave}>
                  <TableCell><Typography variant="body2" fontWeight={600}>{item.descripcion}</Typography><Typography variant="caption" color="text.secondary">{item.tipo}</Typography></TableCell>
                  <TableCell><TextField type="number" size="small" value={item.cantidad} onChange={(e) => cambiarCantidad(item.clave, e.target.value)} inputProps={{ min: 1, style: { padding: '6px 8px' } }} /></TableCell>
                  ...
                  <TableCell><IconButton size="small" color="error" onClick={() => setCarrito((a) => a.filter((fila) => fila.clave !== item.clave))}><DeleteOutlineOutlinedIcon fontSize="small" /></IconButton></TableCell>
                </TableRow>)}
                {carrito.length === 0 ? <TableRow><TableCell colSpan={5} align="center" sx={{ py: 5, color: 'text.secondary' }}>Agrega servicios, productos u otros conceptos para construir la venta.</TableCell></TableRow> : null}
              </TableBody>
            </Table>
          </Seccion>
        ...
        <Stack spacing={2}>
          <Seccion titulo="Resumen">
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: 1, alignItems: 'center', mt: 1 }}><Typography variant="body2">Descuento</Typography><TextField size="small" type="number" value={descuento} onChange={(e) => setDescuento(e.target.value)} inputProps={{ min: 0 }} /></Box>
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: 1, alignItems: 'center', mt: 1 }}><Typography variant="body2">Impuesto</Typography><TextField size="small" type="number" value={impuesto} onChange={(e) => setImpuesto(e.target.value)} inputProps={{ min: 0 }} /></Box>
            <Divider sx={{ my: 1.5 }} />
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 1.5, p: 1.5 }}><Typography variant="h6" fontWeight={800}>Total</Typography><Typography variant="h5" fontWeight={800} color="primary.main">{dinero(total)}</Typography></Box>
          </Seccion>
          ...
          <Seccion titulo="Cobro">
            ...
            {metodoPago !== 'EFECTIVO' ? <TextField label="Referencia" size="small" fullWidth value={referencia} onChange={(e) => setReferencia(e.target.value)} sx={{ mt: 1.25 }} /> : null}
            <Stack spacing={1} sx={{ mt: 1.5 }}>
              <Button variant="contained" disabled={guardando || carrito.length === 0 || !turno?.id} onClick={() => { setModoCobro(true); guardar(true); }} sx={dbanuStyles.addButtonRevive} startIcon={<ReceiptLongOutlinedIcon />}>Guardar y cobrar</Button>
              <Button variant="outlined" disabled={guardando || carrito.length === 0 || !turno?.id} onClick={() => guardar(false)}>Guardar pendiente</Button>
            </Stack>
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1.25 }}>El cobro queda asociado automáticamente a {turno?.caja_nombre || 'la caja'} y al turno actual.</Typography>
          </Seccion>
        </Stack>
      </Box>
...
function Seccion({ titulo, children }) {
  return <Paper elevation={0} sx={{ p: 1.75, border: '1px solid #e2e8f0', borderRadius: 2, bgcolor: '#fff' }}><Typography variant="subtitle2" fontWeight={800} sx={{ mb: 1.25 }}>{titulo}</Typography>{children}</Paper>;
}

function DatoOperacion({ label, valor, destacado = false }) {
  return <Box sx={{ px: 1.5, py: 0.5, minWidth: 0, borderRight: { md: '1px solid #e2e8f0' } }}><Typography variant="caption" color="text.secondary" display="block">{label}</Typography>{destacado ? <Chip size="small" label={valor} color="warning" variant="outlined" sx={{ mt: 0.5, fontWeight: 700 }} /> : <Typography variant="body2" fontWeight={700} noWrap title={valor}>{valor}</Typography>}</Box>;
}

function ResumenFila({ label, valor }) {
  return <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.55 }}><Typography variant="body2" color="text.secondary">{label}</Typography><Typography variant="body2" fontWeight={700}>{valor}</Typography></Box>;
}
