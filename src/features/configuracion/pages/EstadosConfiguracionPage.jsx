...
export function EstadosConfiguracionPage() {
  ...
  const handleGuardar = async () => {
    try {
      if (!formData.codigo || !formData.entidad || !formData.nombre) {
        showNotificacion('Complete código, entidad y nombre', 'warning');
        return;
      }
    ...
    return (
      <Box className="page-wrapper">
        <PageHeader
          ...
          descripcion="Configura la presentación del estado. Los códigos protegidos siguen controlados por las reglas del backend."
            ...
            <Box sx={formStyles.seccion}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 1.5 }}>
                <TextField
                  ...
                  size="small"
                  disabled={protegido}
                  helperText={protegido ? 'Código protegido por reglas del sistema.' : 'Ej.: MEM_EN_REVISION'}
                />
                <TextField
                  label="Entidad"
                  name="entidad"
                  ...
                  size="small"
                  disabled={protegido}
                  helperText="Ej.: MEMBRESIA, VENTA, PAGO"
                />
  ...
  return (
    <Box className="page-wrapper">
      <PageHeader
        titulo="Estados"
        descripcion="Catálogos de estados por entidad. Los códigos del sistema se protegen; nombre, color, orden y presentación se administran aquí."
        icono={<TuneOutlinedIcon />}
      />
      <Paper className="page-content-container" elevation={0}>
        <GestionToolbar
          total={items.length}
          busqueda={busqueda}
          onBusqueda={(valor) => { setBusqueda(valor); cargar({ busqueda: valor }); }}
          acciones={<Button startIcon={<AddOutlinedIcon />} onClick={handleNuevo} sx={dbanuStyles.addButtonRevive}>Añadir</Button>}
        />

        <Box sx={{ px: 2, pb: 1.5, maxWidth: 320 }}>
          <TextField select fullWidth size="small" label="Entidad" value={entidad} onChange={(e) => { setEntidad(e.target.value); cargar({ entidad: e.target.value }); }}>
            <MenuItem value="">Todas</MenuItem>
            {entidades.map((item) => <MenuItem key={item} value={item}>{item}</MenuItem>)}
          </TextField>
        </Box>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell align="center">Protegido</TableCell>
                <TableCell align="center">Acciones</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {!cargando && items.length === 0 ? (
                <TableRow><TableCell colSpan={8} align="center">No hay estados registrados.</TableCell></TableRow>
              ) : items.map((item) => (
                <TableRow key={item.id} hover>
                  <TableCell>{item.codigo}</TableCell>
                  <TableCell>{item.entidad}</TableCell>
                  <TableCell>{item.nombre}</TableCell>
                  <TableCell><StatusChip estado={item.activo ? item.nombre : 'Inactivo'} /></TableCell>
                  ...
                  <TableCell align="center">
                    <Tooltip title="Editar">
                      <IconButton size="small" onClick={() => handleEditar(item)} sx={dbanuStyles.actionEdit}><EditOutlinedIcon sx={{ fontSize: 18 }} /></IconButton>
                    </Tooltip>
                    {item.activo && !item.protegido_sistema ? (
                      <Tooltip title="Desactivar">
                        <IconButton size="small" onClick={() => handleDesactivar(item)} sx={dbanuStyles.actionDelete}><PowerSettingsNewOutlinedIcon sx={{ fontSize: 18 }} /></IconButton>
                      </Tooltip>
                    ) : null}
                  </TableCell>
                </TableRow>
