...
function Formulario({ tipo, formData, catalogos, onChange, setFormData, onNotificar }) {
  ...
  if (tipo === 'productos') {
    ...
    const agregarLote = () => setFormData((actual) => ({
      ...actual,
      lotes: [...(actual.lotes || []), { id: null, sede_id: sedes[0]?.id || '', codigo_lote: '', fecha_elaboracion: '', fecha_ingreso_inventario: new Date().toISOString().slice(0, 10), fecha_vencimiento: '', cantidad_inicial: 0, stock_actual: 0, costo_unitario: '', activo: true }],
    }));
            ...
            {sedes.map((sede) => {
              ...
              return (
                <Box key={sede.id} sx={{ py: 1.2, display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'minmax(180px,1.4fr) 1fr 1fr 1fr' }, gap: 1, alignItems: 'center' }}>
                  ...
                  <TextField label="Stock actual" type="number" size="small" value={stock.stock_actual ?? 20} onChange={(e) => actualizarSede('stocks_sede', sede.id, 'stock_actual', e.target.value)} disabled={!formData.controla_stock} />
          ...
          <Box sx={{ border: '1px solid #e2e8f0', borderRadius: 2, overflow: 'hidden' }}>
            ...
            <Stack divider={<Divider flexItem />} sx={{ px: 1.4 }}>
              {(formData.lotes || []).map((lote, indice) => (
                <Box key={lote.id || `nuevo-${indice}`} sx={{ py: 1.2, display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1.1fr 1fr 1fr 1fr 1fr 1fr 1fr auto' }, gap: 1, alignItems: 'center' }}>
                  ...
                  <TextField label="Ingreso a Revive" type="date" size="small" value={lote.fecha_ingreso_inventario || ''} onChange={(e) => actualizarLote(indice, 'fecha_ingreso_inventario', e.target.value)} slotProps={{ inputLabel: { shrink: true } }} />
