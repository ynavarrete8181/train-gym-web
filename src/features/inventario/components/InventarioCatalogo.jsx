...
function Formulario({ tipo, formData, catalogos, onChange, setFormData, onNotificar }) {
  ...
  if (tipo === 'productos') {
    ...
    const agregarLote = () => setFormData((actual) => ({
      ...actual,
      lotes: [...(actual.lotes || []), { id: null, sede_id: sedes[0]?.id || '', codigo_lote: '', fecha_vencimiento: '', cantidad_inicial: 0, stock_actual: 0, costo_unitario: '', activo: true }],
    }));
          ...
          <Box sx={{ border: '1px solid #e2e8f0', borderRadius: 2, overflow: 'hidden' }}>
            ...
            <Stack divider={<Divider flexItem />} sx={{ px: 1.4 }}>
              {(formData.lotes || []).map((lote, indice) => (
                <Box key={lote.id || `nuevo-${indice}`} sx={{ py: 1.2, display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr 1fr 1fr auto' }, gap: 1, alignItems: 'center' }}>
